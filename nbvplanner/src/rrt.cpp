...
#include "../include/nbvplanner/octomap_utils.h"
...
nbvInspection::RrtTree::RrtTree(mesh::StlMesh * mesh)
{
    mesh_ = mesh;
    kdTree_ = kd_create(3);
    iterationCount_ = 0;
...
void nbvInspection::RrtTree::iterate(int iterations,std::shared_ptr<octomap::OcTree> &ot_)
{
    ...
    if (octoutils::CellStatus::kFree
            == octoutils::getLineStatusBoundingBox(
                origin, direction + origin + direction.normalized() * params_.dOvershoot_,
                params_.boundingBox_,ot_)
        ...
        newNode->gain_ = newParent->gain_
                + gain(newNode->state_,ot_) * exp(-params_.degressiveCoeff_ * newNode->distance_);
...
void nbvInspection::RrtTree::initialize(std::shared_ptr<octomap::OcTree> &ot)
{
    ...
    for (typename std::vector<StateVec>::reverse_iterator iter = bestBranchMemory_.rbegin();
        ...
        if (octoutils::CellStatus::kFree
            ...
            newParent->children_.push_back(newNode);
            newNode->gain_ = newParent->gain_
                    + gain(newNode->state_,ot) * exp(-params_.degressiveCoeff_ * newNode->distance_);
...
double nbvInspection::RrtTree::gain(StateVec state,std::shared_ptr<octomap::OcTree> &ot)
{
    // This function computes the gain
    double gain = 0.0;
    const double disc = ot->getResolution();
    ...
    for (vec[0] = std::max(state[0] - params_.gainRange_, params_.minX_);
        ...
        for (vec[1] = std::max(state[1] - params_.gainRange_, params_.minY_);
            ...
            for (vec[2] = std::max(state[2] - params_.gainRange_, params_.minZ_);
                ...
                double probability;
                octoutils::CellStatus node = octoutils::getCellProbabilityPoint(
                            vec, &probability,ot);
                if (node == octoutils::CellStatus::kUnknown) {
                    // Rayshooting to evaluate inspectability of cell
                    if (octoutils::CellStatus::kOccupied
                            != octoutils::getVisibility(origin, vec, false,ot)) {
                        gain += params_.igUnmapped_;
                        // TODO: Add probabilistic gain
                        // gain += params_.igProbabilistic_ * PROBABILISTIC_MODEL(probability);
                    }
                } else if (node == octoutils::CellStatus::kOccupied) {
                    // Rayshooting to evaluate inspectability of cell
                    if (octoutils::CellStatus::kOccupied
                            != octoutils::getVisibility(origin, vec, false,ot)) {
